import { quiz_nintendo } from "./question2.js"; // Import des questions
...
function updateProgressBar() {
  const totalQuestions = quiz_nintendo.questions.length - 1;
...
suivant.addEventListener("click", () => {
  ...
  if (currentQuestionIndex < quiz_nintendo.questions.length) {
    ...
    rejouer.disabled = false;
     questions.innerText = `Vous avez obtenu ${score} points sur ${quiz_nintendo.questions.length}`;
  }
});
...
function checkAnswer (reponse) {
  const bonneReponse = quiz_nintendo.questions[currentQuestionIndex].correct_answer
